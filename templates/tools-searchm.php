...
<div id="search" class="s-search mx-auto my-4">
        ...
        <div class="s-type">
            ...
            <div class="s-type-list">
                <?php foreach ($search_list as $value) {
                    echo '<label for="m_'.$value['default'].'">'.$value['name'].'</label>';
                } ?>
            </div>
        </div>
    ...
    <form action="<?php echo home_url() ?>?s=" method="get" target="_blank" class="super-search-fm">
        <input type="text" id="m_search-text" class="form-control search-key" autocomplete="off" placeholder="<?php _e( '输入关键字搜索', 'i_theme' ); ?>" style="outline:0">
