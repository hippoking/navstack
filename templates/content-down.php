                    ...
                    <div class="d-md-flex mt-n3 mb-5 my-xl-0">
                        ...
                        <div class="app-info">
                            ...
                            <p class="text-muted   mb-4">
                                ...
                                <span class="info-term mr-3"><?php _e('分类标签：','i_theme') ?><?php the_terms( get_the_ID(), 'favorites','<span class="mr-2">', '</span> <span class="mr-2">', '</span>' ); ?><?php the_terms( get_the_ID(), 'sitetag','<span class="mr-2">', '</span> <span class="mr-2">', '</span>' ); ?></span>
                        ...
                        <div id="carousel" class="carousel slide" data-ride="carousel"> 
                            <div class="carousel-inner" role="listbox"> 
                                ...
                                <div class="carousel-item <?php echo $i==0?'active':'' ?>"> 
                                    ...
                                    <?php if(io_get_option('lazyload')): ?>
                                    <img src="" data-src="<?php echo wp_get_attachment_image_src( $down_screen[$i], 'full' )[0] ?>" class="img-fluid lazy" alt="<?php echo $name ?><?php _e('使用截图','i_theme') ?>">
                                    <?php else: ?>
                                    <img src="<?php echo wp_get_attachment_image_src( $down_screen[$i], 'full' )[0] ?>" class="img-fluid" alt="<?php echo $name ?><?php _e('使用截图','i_theme') ?>">
                                    <?php endif ?>
                                    </div> 
                                </div> 
                                <?php }?>
                            ...
                            <div class="down_btn_list text-center my-4">
								<?php if($down_list){
									for($i=0;$i<count($down_list);$i++){
										echo '<div class="my-3"><a class="btn btn-danger custom_btn-d mx-auto down_count" href="'.(io_get_option('is_go')? home_url().'/go/?url='.base64_encode($down_list[$i]['down_btn_url']) : $down_list[$i]['down_btn_url']).'" target="_blank" data-id="'. get_the_ID() .'" data-action="down_count" data-mmid="down-mm-'.$i.'">'.$down_list[$i]['down_btn_name'].'</a>';
                                        if($down_list[$i]['down_btn_info']) echo '<p>'. $down_list[$i]['down_btn_info'] .'</p>';
                                        if($down_list[$i]['down_btn_tqm']) echo '<p class="text-muted text-sm down-mm-'.$i.'">'.__('提取码：','i_theme').$down_list[$i]['down_btn_tqm'].'</p><input type="text" style="width:1px;position:absolute;height:1px;background:transparent;border:0px solid transparent" name="down-mm-'.$i.'" value="'.$down_list[$i]['down_btn_tqm'].'" id="down-mm-'.$i.'">';
										echo '</div>';
									}
								}else{
