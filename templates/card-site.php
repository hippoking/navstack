<?php if ( ! defined( 'ABSPATH' ) ) { exit; }  ?>
        ...
        <div class="url-body default">    
            <a href="<?php echo $url ?>" <?php echo $blank ?> data-id="<?php echo $post->ID ?>" data-url="<?php echo rtrim($link_url,"/") ?>" class="card no-c <?php echo $is_views ?> mb-4 site-<?php echo $post->ID ?>" <?php echo $tooltip . ' ' . $is_html ?> title="<?php echo $title ?>">
                <div class="card-body">
                <div class="url-content d-flex align-items-center">
                    ...
                    <div class="url-info flex-fill">
                        ...
                        <p class="overflowClip_1 m-0 text-xs"><?php echo htmlspecialchars(get_post_meta($post->ID, '_sites_sescribe', true)) ?: preg_replace("/(\s|\&nbsp\;|　|\xc2\xa0)/","",get_the_excerpt($post->ID)); ?></p>
                    </div>
                </div>
                </div>
            </a> 
            <?php if( $link_url!="" && io_get_option("togo") && io_get_option("details_page") ) { ?>
            <a href="<?php echo io_get_option('is_go')? home_url().'/go/?url='.base64_encode($link_url) : $link_url ?>" class="togo text-center text-muted is-views" target="_blank" data-id="<?php echo $post->ID ?>" data-toggle="tooltip" data-placement="right" title="<?php _e('直达','i_theme') ?>" rel="external nofollow"><i class="iconfont icon-goto"></i></a>
            <?php } ?>
        </div>
