	    					...
	    					<div class="col mt-4 mt-sm-0">
	    						<div class="site-body text-sm">
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <?php the_terms( get_the_ID(), 'sitetag',__('标签：','i_theme').'<span class="mr-1">', '</span> <span class="mr-1">', '</span>' ); ?>
	    								<div class="site-go mt-3">
                                            <?php if($m_link_url!=""): ?>
                                                <?php security_check($m_link_url); ?>
                                                <span class="site-go-url">
                                                <a style="margin-right: 10px;" href="<?php echo io_get_option('is_go')? home_url().'/go/?url='.base64_encode($m_link_url) : $m_link_url ?>" title="<?php echo $sitetitle ?>" target="_blank" class="btn btn-arrow"><span><?php _e('链接直达','i_theme') ?><i class="iconfont icon-arrow-r-m"></i></span></a>
                                                <?php if($spare_link =get_post_meta(get_the_ID(),'_spare_sites_link', true)) { ?>
                                                <span class="spare-url card">
                                                    <?php for ($i=0;$i<count($spare_link);$i++) { ?>
                                                    <a class="my-2 mx-3" href="<?php echo io_get_option('is_go')? home_url().'/go/?url='.base64_encode($spare_link[$i]['spare_url']) : $spare_link[$i]['spare_url'] ?>" title="<?php echo $spare_link[$i]['spare_note'] ?>" target="_blank"><span><?php echo $spare_link[$i]['spare_name'] ?><i class="iconfont icon-arrow-r-m"></i></span></a>
                                                    <?php } ?>
                                                </span>
                                                <?php } ?>
                                                </span>
